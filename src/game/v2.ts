...
function addLog(session: GameSessionV2, message: string): void {
  session.logs = [`${dayLabel(session.day)}：${message}`, ...session.logs].slice(0, 80)
  session.player.log = [message, ...session.player.log].slice(0, 80)
}
...
function syncCalendar(session: GameSessionV2): void {
  ...
  if (session.day > 1 && (session.day - 1) % 360 === 0) {
    session.player.age += 1
  }
}
...
export function resumeSession(session: GameSessionV2): GameSessionV2 {
  const next = clone(session)
  if (!next.pendingEvent && !next.death) {
    next.paused = false
...
function executeDailyActivity(session: GameSessionV2, activity: DayActivity): { cultivation: number; stones: number; healing: number } {
  ...
  if (activity === 'cultivate') {
    ...
    const dwellingHealing = 1 + Math.floor(session.player.dwelling.level / 2)
    healing = lowerInjury(session, dwellingHealing)
  }
...
function buildEvent(session: GameSessionV2, activity: DayActivity): LiveEvent | null {
  ...
  const chance = { cultivate: 0.1, work: 0.14, forage: 0.32, travel: 0.38, recover: 0.05 }[activity]
  if (rng() > chance) return null
  const roll = rng()
  const sharedLeave: LiveEventChoice = { id: 'leave', label: '谨慎离去', description: '不争机缘，保全当前状态。', stat: 'daoxin' }

  if (activity === 'forage' && roll < 0.48) {
    return { id: `herb-${session.day}`, tier: '机会', title: '山涧灵草', description: '雾气深处有一株灵草散发微光，周围似乎有兽类活动的痕迹。', threat: 18, reward: 12, choices: [{ id: 'scout', label: '先行辨识', description: '以悟性与气运确认药性及周围危险。', stat: 'wuxing' }, { id: 'force', label: '强取灵草', description: '直接采摘，可能惊动暗处妖兽。', stat: 'power', risky: true }, sharedLeave] }
  }
  if ((activity === 'forage' || activity === 'travel') && roll < 0.76) {
    return { id: `beast-${session.day}`, tier: '风险', title: '妖兽拦路', description: '一头赤鬃妖狼自林间现身，灵压远胜寻常野兽。', threat: 25 + Math.floor(rng() * 12), reward: 18, choices: [{ id: 'force', label: '正面迎战', description: '以战力强行开路，战败可能留下重伤。', stat: 'power', risky: true }, { id: 'scout', label: '观察弱点', description: '寻找妖兽的行动空隙，再决定去留。', stat: 'wuxing' }, sharedLeave] }
  }
  if (activity === 'work') {
    return { id: `market-${session.day}`, tier: '机会', title: '坊市压价', description: '一位商人看中了你的货品，却试图以远低于市价的灵石成交。', threat: 16, reward: 14, choices: [{ id: 'negotiate', label: '据理交涉', description: '以道心和见闻争取合理价码。', stat: 'daoxin' }, { id: 'scout', label: '洞察行情', description: '用气运与判断寻找更好的买主。', stat: 'qiyun' }, sharedLeave] }
  }
  if (activity === 'cultivate') {
    return { id: `insight-${session.day}`, tier: '机会', title: '灵机一闪', description: '吐纳间天地灵气微微共鸣，似有一缕感悟即将消散。', threat: 14, reward: 10, choices: [{ id: 'scout', label: '凝神参悟', description: '以悟性捕捉那一瞬灵机。', stat: 'wuxing' }, { id: 'force', label: '强行冲关', description: '压榨经脉争取更大收获，可能加重伤势。', stat: 'power', risky: true }, sharedLeave] }
  }
  return { id: `ruin-${session.day}`, tier: '命运', title: '古碑低语', description: '荒野古碑泛起微光，碑后传来与自身天命隐约相连的呼唤。', threat: 30, reward: 28, major: true, choices: [{ id: 'scout', label: '谨慎探查', description: '借道韵和气运解读残缺碑文。', stat: 'qiyun' }, { id: 'force', label: '踏入碑后禁制', description: '强行踏入未知之地，可能触及重大因果。', stat: 'power', risky: true }, sharedLeave] }
}

function eventChance(session: GameSessionV2, event: LiveEvent, choice: LiveEventChoice): number {
  const player = session.player
  const stat = choice.stat === 'power' ? player.power : player.daoJi[choice.stat]
  const injuryPenalty = injuryRank[session.resources.injury] * 8
  const daoYunBonus = (choice.stat === 'wuxing' && player.daoYun.name === '明察秋毫') || (choice.stat === 'qiyun' && player.daoYun.name === '天人感应') || (choice.stat === 'daoxin' && player.daoYun.name === '磐石之志') ? 12 : 0
  const pillBonus = choice.id === 'force' && session.resources.pills > 0 ? 4 : 0
  return clamp(0.25 + (stat + daoYunBonus + pillBonus - event.threat - injuryPenalty) / 70, 0.08, 0.92)
}
...
function resolveEventMutable(session: GameSessionV2, event: LiveEvent, choiceId: EventApproach, automatic = false): void {
  ...
  const chance = eventChance(session, event, choice)
  const success = eventRng(session, `${event.id}:${choice.id}`)() < chance
  if (success) {
    ...
    const dangerous = event.tier === '风险' || event.tier === '命运' || choice.risky
    if (dangerous) {
      ...
      addLog(session, `「${event.title}」未能如愿，但并无实质损失。`)
    }
  }
  session.eventHistory.push(event.id)
  session.pendingEvent = null
...
function maybePauseForEvent(session: GameSessionV2, event: LiveEvent, offline: boolean): boolean {
  ...
  if (event.tier === '日志') return false
  if (event.tier === '风险' || event.tier === '命运') return true
  return session.autoPolicy.pauseOpportunity
}

function advanceOneDay(current: GameSessionV2, offline: boolean, report: OfflineReport): boolean {
  const session = current
  if (session.pendingEvent || session.death) return false
...
export function advanceByWallClock(session: GameSessionV2, now = Date.now()): GameSessionV2 {
  if (session.paused || session.speed === 0 || session.pendingEvent || session.death) return { ...session, lastSyncedAt: now }
...
export function rollbackAfterDeath(session: GameSessionV2): GameSessionV2 {
  ...
  const restored: GameSessionV2 = {
    ...clone(session),
    ...clone(checkpoint.snapshot),
    checkpoints: clone(session.checkpoints),
    death: null,
    pendingEvent: null,
    paused: true,
    speed: 0,
...
export function dismissOfflineReport(session: GameSessionV2): GameSessionV2 {
  return { ...session, offlineReport: null }
}
